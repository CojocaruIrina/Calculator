...
function addDecimal() {
  // If an operator is pressed, don't add decimal
  if (awaitingNextValue) return;
  // add one decimal, if none
  if (!calculatorDisplay.textContent.includes('.')) {
    calculatorDisplay.textContent = `${calculatorDisplay.textContent}.`;
  }
}

// Calculate first and second values depending on operator pressed
const calculate = {
...
function useOperator(operator) {
  ...
  // Assign firstValue if no value is given
  if (!firstValue) {
